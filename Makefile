...
KPT_LIVE_APPLY_ARGS += --reconcile-timeout=5m
KPT_RETRY := 3
endif
...
CODESPACES_ENGINECONFIG_CUSTOM_SETTINGS_PATCH := $(TOP_DIR)/configs/codespaces-4vcpu-ce-custom-settings.yaml
...
patch-codespaces-engineconfig: | $(YQ)
	...
	@ENGINE_CONFIG_FILE="$(KPT_CORE)/engine-config/engineconfig.yaml"; \
		if [ ! -f "$$ENGINE_CONFIG_FILE" ]; then \
			echo "[ERROR] EngineConfig manifest not found at $$ENGINE_CONFIG_FILE"; exit 1; \
		fi; \
